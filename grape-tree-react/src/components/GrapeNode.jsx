...
const GrapeNode = memo(({ data, isConnectable }) => {
  const {
    label,
    country,
    country_code,
  ...
  const getBerryColor = (berryColor) => {
  ...
  return (
    <div 
      ...
      style={{ 
        ...
        borderColor: is_hovered ? '#0056b3' : (is_selected ? '#007bff' : '#ddd'),
        borderWidth: is_hovered ? '4px' : '2px',
    ...
    >
      ...
      <div className="grape-node-content">
        ...
        <span className="grape-name">{formatLabel(label)}</span>
        {sex && (
